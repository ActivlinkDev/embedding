...
router = APIRouter(tags=["CMS Validate Customer"])

# Strapi config for validate-customer collection
STRAPI_BASE_URL = os.getenv("STRAPI_VALIDATE_CUSTOMER_URL", "https://strapi-production-5603.up.railway.app/api/validate-customer")
...
# Mongo config (reuse locale mapping)
...
async def cms_validate_customer(
    ...
    """
    Proxies Validate Customer content query to Strapi using strapi_locale mapping.
    Falls back to default error if locale is unmapped.
    """
