...
def run_serp_and_update(master_id: str, title: str, locale: str, category: str, model: str, extra: dict = None):
    """
    Background task: call ScaleSERP, build the real locale block and replace the placeholder in the master doc.
    """
    try:
        real_block = build_locale_data_from_serp(title, locale, category, model, extra=extra)
        add_serp_match_flag(real_block, model)

        # Replace the placeholder locale block (which has serp_pending=True) with the real block
        try:
            oid = ObjectId(master_id)
        except Exception:
            print(f"[SERP background] invalid master_id {master_id}")
            return

        # Remove any placeholder block for this locale
        master_collection.update_one(
            {"_id": oid},
            {"$pull": {"Locale_Specific_Data": {"locale": locale, "serp_pending": True}}}
        )

        # Add the real block
        master_collection.update_one(
            {"_id": oid},
            {"$addToSet": {"Locale_Specific_Data": real_block}}
        )

    except Exception as e:
        print(f"[SERP background error] {e}")
...
def create_master_sku(data: MasterSKURequest, background_tasks: Optional[BackgroundTasks] = None, _: None = Depends(verify_token)):
    ...
    if existing:
        ...
        # Fetch localized Icecat info if possible (we will enqueue SERP enrichment)
        ...
        # Create a lightweight placeholder locale block; background task will replace it
        ...
        if extra:
            placeholder_block.update(extra)
        placeholder_block["Serp_match"] = False

        updated = update_existing_sku(existing, data, placeholder_block)
    ...
    # Create a placeholder locale block and enqueue SERP enrichment in background
    locale_info = fetch_locale_info(data.locale) or {}
    currency_code = locale_info.get("currency")
    placeholder_block = {
        "locale": data.locale,
    ...
    doc = {
        ...
        "Locale_Specific_Data": [placeholder_block]
    }
