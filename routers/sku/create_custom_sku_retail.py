...
# This file is a retail-specific duplicate of create_custom_sku.py. The
# difference is that `Locale_Details` is required and its `Price` field is
# mandatory (non-optional) for retail flows.
...
# Retail-specific LocaleDetails: Price is mandatory
...
class CustomSKURequest(BaseModel):
    ...
    # Locale_Details is required for the retail endpoint and must include Price
    Locale_Details: LocaleDetails
    Global_Promotion: Optional[str] = None
    addSERP: Optional[bool] = False


@router.post("/create_custom_sku_retail")
async def create_custom_sku_retail(data: CustomSKURequest, request: Request, _: None = Depends(verify_token)):
    # 0. Validate inputs
    ...
    if existing:
        ...
        if locale_match:
            ...
            locale_details = data.Locale_Details
